...
class VideoMMEDataset(Dataset):
    ...
    def __init__(self, video_folder, subtitle_folder, data_list, processor, feat_folder=None):
    ...
    def __getitem__(self, idx):
        ...
        if self.feat_folder is not None:
            video_tensor = np.load(os.path.join(self.feat_folder, f'{video_name}.npy'))
...
def load_parquet(parquet_file, target_duration=None):
    ...
    for record in df.itertuples():
        if target_duration is None or record.duration == target_duration:
...
def build_videomme_eval(args, processor):
    ...
    dataset = VideoMMEDataset(args.video_folder, args.subtitle_folder, questions, processor, feat_folder=args.feat_folder)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--feat-folder', help='Directory containing video files.', required=True)
    ...
    parser.add_argument("--sub_type", type=str, default=None)
