...
class STCConnector(nn.Module):
    ...
    def __init__(self, config, downsample=(1, 2, 2), depth=4, mlp_depth=2):
        ...
        self.hidden_size = hidden_size = config.mm_hidden_size
        # self.output_hidden_size = output_hidden_size = config.hidden_size
        self.output_hidden_size = output_hidden_size = config.mm_hidden_size
    ...
    def forward(self, x):
        ...
        x = einops.rearrange(x, "t d h w -> t (h w) d")
