...
import sys
sys.path.append('./')
from bvlm.model import *
from bvlm.constants import NUM_FRAMES, IGNORE_INDEX, MODAL_INDEX_MAP
from bvlm.mm_utils import tokenizer_multimodal_token, process_video, process_image
from bvlm.bvlm_trainer import (BVLMTrainer,
...
class ModelArguments:
    # LLM Arguments
    model_type: Optional[str] = field(default="bvlm_qwen", metadata={"help": "Model type selected in the list: " + ", ".join(VLLMs.keys())})
    ...
    for source in sources:
        for sentence in source:
            if modal_token in sentence['value']:
                ...
                prompt = sentence['value'].replace(modal_token, '').strip()
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            modal_token = "<image>"
            sources, prompt = preprocess_multimodal(copy.deepcopy([e["conversations"] for e in sources]), self.data_args, modal_token)
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        model.get_model().initialize_attention_modules(model_args=model_args, for_eval=False)

        vision_tower = model.get_vision_tower()
        vision_tower.to(dtype=torch.bfloat16 if training_args.bf16 else torch.float16, device=training_args.device)

        # model.get_model().vlm_att_encoder_spatial.to(dtype=torch.bfloat16 if training_args.bf16 else torch.float16, device=training_args.device)
        model.get_model().vlm_att_encoder_temporal.to(dtype=torch.bfloat16 if training_args.bf16 else torch.float16, device=training_args.device)
    ...
    # select a Trainer
    trainer = BVLMTrainer(model=model, tokenizer=tokenizer, args=training_args, **data_module)
