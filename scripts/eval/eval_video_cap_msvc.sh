...
if [ ! -f "$output_file" ]; then
    for IDX in $(seq 0 $((CHUNKS-1))); do
        ...
        TRANSFORMERS_OFFLINE=1 CUDA_VISIBLE_DEVICES=${gpu_devices} python3 bvlm/eval/inference_video_cap_msvc.py \
          --model-path ${CKPT} \
...
python3 bvlm/eval/eval_video_cap_msvc_correctness.py \
...
python3 bvlm/eval/eval_video_cap_msvc_detailedness.py \
    --pred-path $output_file \
