...
if [ ! -f "$output_file" ]; then
    for IDX in $(seq 0 $((CHUNKS-1))); do
        ...
        TRANSFORMERS_OFFLINE=1 CUDA_VISIBLE_DEVICES=${gpu_devices} python3 bvlm/eval/inference_video_mcqa_egoschema.py \
            --model-path ${CKPT} \
            --video-folder ${EVAL_DATA_DIR}/egoschema/videos \
